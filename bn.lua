...
-- O hook do autoclick bypass é aplicado após a UI carregar (ver MiscTab)
local autoclickBypassStatus = "Aguardando..."
...
-- ==================== CANCEL / SPAM ====================
local _spamEnabled       = false
local _packetBurst       = 10

local _AbilityRemote     = nil
local _AbilityHandler    = nil
local _ClientDebounce    = nil
local _AbilityData       = nil

local _isPlayerProtected -- forward declaration
local GROUP_PROTECTED_ID = 35357509
local _groupCache        = {}

pcall(function()
	_AbilityData    = require(ReplicatedStorage:WaitForChild("ModuleScripts"):WaitForChild("Data"):WaitForChild(
		"AbilityData"))
	_AbilityHandler = require(LocalPlayer.PlayerScripts:WaitForChild("ModuleScripts"):WaitForChild("AbilityHandler"))
	_ClientDebounce = require(LocalPlayer.PlayerScripts:WaitForChild("ModuleScripts"):WaitForChild("ClientDebounce"))
	_AbilityRemote  = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("AbilityService"):WaitForChild("ToServer")
		:WaitForChild("AbilityActivated____")
end)

local function _getSpamTarget()
	...
	for _, plr in ipairs(Players:GetPlayers()) do
		...
		if _isPlayerProtected and _isPlayerProtected(plr) then continue end
...
RunService.Heartbeat:Connect(function()
	if not _spamEnabled then return end
	if not _AbilityHandler or not _ClientDebounce or not _AbilityRemote then return end
	local ability = _AbilityHandler.activeAbility
	if not ability or not ability._name then return end
	local name = ability._name
	local timeLeft = _ClientDebounce.getTimeLeft(name)
	if not timeLeft or timeLeft <= 0 then
		local target = _getSpamTarget()
		if not target then return end
		for i = 1, _packetBurst do
			_AbilityRemote:FireServer(target)
		end
		ability._isHolding = true
		if ability.activated then
			task.spawn(function() ability:activated() end)
		end
		local data = _AbilityData and _AbilityData[name]
		local cd = data and data.cooldown or 0.5
		_ClientDebounce.set(name, cd)
	end
end)
...
-- Define _isPlayerProtected agora que friendWhitelist existe
_isPlayerProtected = function(plr)
	if not plr then return false end
	if plr == LocalPlayer then return true end
	if friendWhitelist[plr.Name] then return true end
	if _groupCache[plr.UserId] ~= nil then return _groupCache[plr.UserId] end
	local ok, inGroup = pcall(function() return plr:IsInGroup(GROUP_PROTECTED_ID) end)
	local res = ok and inGroup == true
	_groupCache[plr.UserId] = res
	return res
end
...
local function shouldIgnoreHitboxName(name)
	if name == LocalPlayer.Name or friendWhitelist[name] == true then return true end
	local plr = Players:FindFirstChild(name)
	if plr and _isPlayerProtected(plr) then return true end
	return false
end
...
local SpamTab      = Window:CreateTab("Spam", "zap")
local VisualsTab   = Window:CreateTab("Visuals", "eye")
local WhitelistTab = Window:CreateTab("Whitelist", "shield")
local MiscTab      = Window:CreateTab("Misc", "settings")

-- ==================== SPAM TAB ====================
SpamTab:CreateSection("Spam")

SpamTab:CreateParagraph({
	Title = "Como usar",
	Content = "Ativa o cancel automático [R]",
})

local _spamToggleRef
local _skipSpamNotif = false

_spamToggleRef = SpamTab:CreateToggle({
	Name = "Ativar Spam",
	CurrentValue = false,
	Callback = function(state)
		_spamEnabled = state
		if _skipSpamNotif then return end
		Rayfield:Notify({
			Title = "SPAM",
			Content = state and "✅" or "❌",
			Duration = 1,
...
-- Keybind R para toggle do Spam
UserInputService.InputBegan:Connect(function(input, gpe)
	if gpe then return end
	if input.KeyCode == Enum.KeyCode.R then
		_spamEnabled = not _spamEnabled
		if _spamToggleRef then
			_skipSpamNotif = true
			pcall(function() _spamToggleRef:Set(_spamEnabled) end)
			_skipSpamNotif = false
		end
		Rayfield:Notify({
			Title = "SPAM",
			Content = _spamEnabled and "✅" or "❌",
			Duration = 0.8,
			Image = 94733361910796,
		})
	end
...
local _bypassParagraphRef = MiscTab:CreateParagraph({
	Title = "Autoclicker Bypass",
	Content = "Aguardando...",
})

-- Aplica o hook DEPOIS que a UI carregou, sem bloquear execução
task.spawn(function()
	task.wait(0.5) -- garante que a UI já foi renderizada
	local ok, err = pcall(function()
		local OldNamecall
		OldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
			local method = getnamecallmethod()
			if method == "FireServer" and
				self == ReplicatedStorage.Remotes.GameServices.ToServer.AutoclickerDetected
			then
				return
			end
	...
	autoclickBypassStatus = ok and "✅ Ativado" or ("❌ Executor incompatível: " .. tostring(err))
	-- Tenta atualizar o parágrafo via método :Set do Rayfield
	local updated = false
	pcall(function()
		_bypassParagraphRef:Set("Autoclicker Bypass", autoclickBypassStatus)
		updated = true
	end)
	-- Fallback: busca o TextLabel do parágrafo diretamente no CoreGui
	if not updated then
		for _, gui in ipairs(game:GetService("CoreGui"):GetDescendants()) do
			if gui:IsA("TextLabel") and gui.Text == "Aguardando..." then
				pcall(function() gui.Text = autoclickBypassStatus end)
				break
			end
		end
	end
